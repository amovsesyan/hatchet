...
def get_palette(trace_events: pd.DataFrame, scale=None):
    funcs = trace_events
    # names = funcs["Name"].unique().tolist()
    names = reversed(trace_events["group_name"].unique().tolist())
    depths = (
        funcs.groupby("group_name", observed=True)["_depth"]
...
def get_factor_cmap(field_name, trace_events, **kwargs):
    palette = get_palette(trace_events, **kwargs)
